...
train_ratio_imageNet2Face = 200

save_dir = './Experiment_3_Final/'
...
#            loss_Face_vl=sess.run(loss_Face,feed_dict={keep_ratio:1})
#            loss_imageNet_vl=sess.run(loss_imageNet,feed_dict={keep_ratio:1})
#            print('\rbatch={:d}/{:d},loss_Face={:f},loss_imageNet={:f},used_time:{:.2f}s'.format(batch_count, nb_batches,loss_Face_vl, loss_imageNet_vl, time.time()-curr_time),end=' ')
##            print('\rbatch={:d}/{:d},used_time:{:.2f}s'.format(batch_count, nb_batches, time.time()-curr_time),end=' ')
#            curr_time=time.time()
#    print('\n')
...
def generate_merge_list_conv():
    ...
    #    dataset_imageNet_train, dataset_imageNet_val, nb_exp_imageNet_train, nb_exp_imageNet_val=load_imageNet(BATCH_SIZE_HESSIAN,CPU_CORES)
    ...
    #    hessian_inverse_B=run_hessian(hessian_op_B,iterator_imageNet,dataset_imageNet_train,nb_exp_imageNet_train,nb_batches_hessian,BATCH_SIZE_HESSIAN)
        ...
        for j in range(kernel_nb):
            for kernel_i in range(kernel_size):
                for kernel_j in range(kernel_size):
                    dist_vec = Wb_B[kernel_i, kernel_j, :, i] - Wb_A[kernel_i, kernel_j, :, j]
                    #                    delta_A=np.dot(MtxA,-dist_vec)
                    merged_vector[kernel_i, kernel_j, :, i, j] = Wb_B[kernel_i, kernel_j, :, j]
                    #                    merged_vector[kernel_i,kernel_j,:,i,j]=(Wb_A[kernel_i,kernel_j,:,i]+Wb_B[kernel_i,kernel_j,:,j])/2
                    err_mtx[i, j, kernel_i, kernel_j] = 0.5 * np.dot(np.dot(dist_vec, hessian_A), dist_vec)
...
def generate_merge_list_fc():
    ...
    #    dataset_Face_train, dataset_Face_val, nb_exp_Face_train, nb_exp_Face_val=load_celebA(BATCH_SIZE_HESSIAN,CPU_CORES)
    ...
    #    hessian_A=run_hessian(hessian_op_A,iterator_Face,dataset_Face_train,nb_exp_Face_train,nb_batches_hessian,BATCH_SIZE_HESSIAN)
    #    hessian_inverse_A=np.linalg.pinv(hessian_A)

    #    hessian_op_B=calculate_hessian_fc_tf(layer_inputs_imageNet[layer_name])
    #    hessian_B=run_hessian(hessian_op_B,iterator_imageNet,dataset_imageNet_train,nb_exp_imageNet_train,nb_batches_hessian,BATCH_SIZE_HESSIAN)
    ...
    for i in range(kernel_nb ** 2):
        ...
        if count == 0:
            ...
            merged_vector[count, :] = Wb_B[:, r_index]
            count += 1
...
for layer_index in range(15):
    ...
    #    loss_imageNet = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(labels=y_true_imageNet, logits=y_imageNet))
    ...
    opt_imageNet = tf.train.MomentumOptimizer(learning_rate=0.01, momentum=0.9, use_nesterov=False)
