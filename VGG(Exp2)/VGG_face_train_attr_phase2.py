...
#%% Create Weights
W_VGG_Face={
        'conv1_1':tf.get_variable('W_VGG_Face_conv1_1', shape=[3,3,3,64]),
        'conv1_2':tf.get_variable('W_VGG_Face_conv1_2', shape=[3,3,64,64]),
        'conv2_1':tf.get_variable('W_VGG_Face_conv2_1', shape=[3,3,64,128]),
        'conv2_2':tf.get_variable('W_VGG_Face_conv2_2', shape=[3,3,128,128]),
        'conv3_1':tf.get_variable('W_VGG_Face_conv3_1', shape=[3,3,128,256]),
        'conv3_2':tf.get_variable('W_VGG_Face_conv3_2', shape=[3,3,256,256]),
        'conv3_3':tf.get_variable('W_VGG_Face_conv3_3', shape=[3,3,256,256]),
        'conv4_1':tf.get_variable('W_VGG_Face_conv4_1', shape=[3,3,256,512]),
        'conv4_2':tf.get_variable('W_VGG_Face_conv4_2', shape=[3,3,512,512]),
        'conv4_3':tf.get_variable('W_VGG_Face_conv4_3', shape=[3,3,512,512]),
        'conv5_1':tf.get_variable('W_VGG_Face_conv5_1', shape=[3,3,512,512]),
        'conv5_2':tf.get_variable('W_VGG_Face_conv5_2', shape=[3,3,512,512]),
        'conv5_3':tf.get_variable('W_VGG_Face_conv5_3', shape=[3,3,512,512]),
...
b_VGG_Face={
        'conv1_1':tf.get_variable('b_VGG_Face_conv1_1', shape=[64]),
        'conv1_2':tf.get_variable('b_VGG_Face_conv1_2', shape=[64]),
        'conv2_1':tf.get_variable('b_VGG_Face_conv2_1', shape=[128]),
        'conv2_2':tf.get_variable('b_VGG_Face_conv2_2', shape=[128]),
        'conv3_1':tf.get_variable('b_VGG_Face_conv3_1', shape=[256]),
        'conv3_2':tf.get_variable('b_VGG_Face_conv3_2', shape=[256]),
        'conv3_3':tf.get_variable('b_VGG_Face_conv3_3', shape=[256]),
        'conv4_1':tf.get_variable('b_VGG_Face_conv4_1', shape=[512]),
        'conv4_2':tf.get_variable('b_VGG_Face_conv4_2', shape=[512]),
        'conv4_3':tf.get_variable('b_VGG_Face_conv4_3', shape=[512]),
        'conv5_1':tf.get_variable('b_VGG_Face_conv5_1', shape=[512]),
        'conv5_2':tf.get_variable('b_VGG_Face_conv5_2', shape=[512]),
        'conv5_3':tf.get_variable('b_VGG_Face_conv5_3', shape=[512]),
...
Weights_VGG_Face, bias_VGG_Face = pickle.load(open('tf_saved_VGG_face/unpruned_weights','rb'))
#%% Define Model Graph
...
while True:
    ...
    while True:
        try:
            ...
            batch_count += 1
            if batch_count % 10 ==0:
                print('\repoch={:d},batch={:d}/{:d},used_time:{:.2f}s'.format(epoch_count,batch_count,nb_batches,time.time()-curr_time),end=' ')
                curr_time=time.time()
...
#        saver.save(sess=sess, save_path=save_path)
        ...
        patience+=1
        improved_str = ''
    print('\nEpoch:{:d}, val_acc={:%}'.format(epoch_count, acc_val)+improved_str)

# If no improvement found in the required number of iterations.
    if patience > require_improvement:
...
#
#config_tuple=open(save_dir+'unpruned_weights_phase2','wb')
